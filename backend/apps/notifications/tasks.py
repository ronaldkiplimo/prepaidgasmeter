...
def send_token_notification_task(token_id: str):
    try:
        token_obj = GasToken.objects.select_related(
            "transaction", "transaction__user", "transaction__payment"
        ).get(id=token_id)
    except GasToken.DoesNotExist:
        logger.error("Token %s not found", token_id)
        return
    ...
    token_obj.status = GasToken.Status.DELIVERED
