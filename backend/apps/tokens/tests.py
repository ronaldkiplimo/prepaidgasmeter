...
@override_settings(
    STRON_API_URL="http://www.server-newv.stronpower.com/api",
    STRON_DIRECT_API_URL="http://www.server-newv.stronpower.com/api",
...
class StronVendingServiceTest(SimpleTestCase):
    ...
    def test_generate_token_uses_manual_vending_payload(self, post):
    ...
    @patch("apps.tokens.services.stron.requests.post")
    def test_generate_token_supports_direct_vending_payload(self, post):
        ...
        post.assert_called_once_with(
            "http://www.server-newv.stronpower.com/api/VendingMeterDirectly",
            json={
                "CompanyName": "Stron-Good",
                "UserName": "Admin01",
                "PassWord": "123456",
    ...
    def test_missing_credentials_raise_clear_error(self):
        with override_settings(STRON_COMPANY_NAME="your-company-name"):
            with self.assertRaisesMessage(ValueError, "Stron credentials are not configured"):
